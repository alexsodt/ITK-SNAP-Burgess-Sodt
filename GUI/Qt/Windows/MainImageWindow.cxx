...
void MainImageWindow::Initialize(GlobalUIModel *model)
{
  ...
  // AJS save polygons
  activateOnFlag(ui->actionSavePolygons, m_Model, UIF_IRIS_WITH_BASEIMG_LOADED);
  activateOnFlag(ui->actionOpenPolygons, m_Model, UIF_IRIS_WITH_BASEIMG_LOADED);
...
void MainImageWindow::on_actionOpenPolygons_triggered()
{
...
void MainImageWindow::on_actionSavePolygons_triggered()
{
  // AJS save polygons
  // This should only happen in SNAP mode
  assert(!m_Model->GetDriver()->IsSnakeModeActive());
...
/*		SliceViewPanel *thePanel = GetSlicePanel(p);
		...
		if( thePanel )
			thePanel->sliceWritePolygonsToBuffer(out);
		else
		{
		}
*/
